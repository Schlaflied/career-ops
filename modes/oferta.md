...
**13. Minimum-Wage Floor Check** (from `advertised_comp` + `templates/minimum-wage.yml`; jurisdiction from the JD's stated location, falling back to `config/profile.yml` → `location` when the JD is silent — same region-aware pattern as the employment-classification signal):
