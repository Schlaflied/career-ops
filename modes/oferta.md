...
**13. Minimum-Wage Floor Check** (from `advertised_comp` + `templates/minimum-wage.yml`; jurisdiction from the JD's stated location ONLY — NEVER from `config/profile.yml` → `location`, which describes the candidate, not the job; remote, relocation, and multi-location postings make that substitution wrong):

Advertised compensation that looks low relative to public minimum-wage reference data is worth surfacing — but `templates/minimum-wage.yml` is a **non-authoritative reference table**, not a verified statement of the current legal minimum wage (see the file's header). Resolve the posting's governing jurisdiction strictly from the JD's own stated work location. If the JD does not state a work location precisely enough to resolve a jurisdiction, or the resolved jurisdiction has no row in the table, **skip this signal entirely** — do not guess, and do not fall back to the candidate's own `config/profile.yml` location.

**Comparable-amount gate (mandatory):** only compare when `advertised_comp` resolves to a **guaranteed, fixed cash amount** in the table row's currency. Exclude: ranges (e.g. "$16-18/hour" has no single figure to compare), and any variable or non-cash component — bonuses, commissions, allowances, overtime pay, 13th-month/holiday pay, and benefits. If `advertised_comp` is `null`, a non-numeric phrase ("competitive"), a range, or otherwise not a guaranteed fixed cash figure, skip this signal — absence or non-fixed comp is the pay-transparency signal's territory, not this one's.

**Rate normalization:** when the fixed cash amount is already hourly, compare it directly against the applicable rate. When it is annual or monthly, convert to hourly using the JD's own stated working hours whenever the JD gives one; only fall back to the conservative assumption of **2080 hours/year** (52 weeks × 40 hours; monthly × 12 first) when the JD is silent on hours, and **always disclose in the output which hours figure was used** (JD-stated or the 2080-hour fallback). If no usable hours figure or currency match is available to complete the conversion, skip this signal rather than comparing on an unreliable assumption.

**12-month staleness gate (mandatory — mirrors the STALENESS RULE in `templates/minimum-wage.yml`):** apply ordinary date-aware rate selection first: use `general_rate` while today's date is before `next_effective`; use `next_rate` once today's date is on or after `next_effective` — an already-arrived `next_effective` date means `next_rate` is the officially confirmed rate in force today, never a sign of staleness. The 12-month `as_of` freshness threshold applies uniformly to every row, including rows with no `next_effective` at all, but only matters when `general_rate` is the rate that would otherwise be selected (i.e. today is before `next_effective`, or there is no `next_effective`): if today's date is more than 12 months past `as_of` in that situation, the row is review-due — treat it as tentative reference data at best, and skip the comparison entirely, since `general_rate` is the only candidate rate and it is the one that has gone stale. A row is never skipped as stale merely because `next_effective` has passed; that condition means `next_rate` now covers today.

**Carve-out honesty:** each row lists `special_rates` (students, homeworkers, etc.) as informational context. Apply a special rate only when **every** eligibility condition the row states for it is explicitly confirmed in the JD — not merely inferred from a job title. An explicit "student" role title alone does NOT establish eligibility for a row's student rate if that rate also carries an age threshold or a maximum weekly-hours condition and the JD is silent on them. When any stated condition is unconfirmed, use the general rate and explicitly note that the carve-out is unverified rather than assuming it applies.

**This fires as a signal, never a legal conclusion,** when the advertised figure (after all the gates above) is below the table's reference rate for the applicable rate line. Because the table is non-authoritative, every rendered instance must route the candidate to verify with a lawyer or an official source rather than asserting the current legal minimum as fact. If all the gates above pass, append a short, non-alarmist note to the report:

> ⚠️ **Minimum-wage floor signal:** [Render in {language.output}: state the arithmetic fact and the table's non-authoritative reference rate with its citation — e.g. a fictional Acme Corp posting in Ontario advertising $15.00/hour reads "advertised $15.00/hour is below the $17.60/hour reference rate this system has on file for Ontario (as of {as_of}, citing ESA 2000) — this is non-authoritative reference data, not a verified current legal minimum" — disclosing which hours figure was used for any conversion, and naming the applicable special rate instead when every one of its eligibility conditions is confirmed in the JD. Then note the common benign-to-bad range of explanations: a stale posting carrying a pre-increase rate, an employer unaware of the current rate, or a fabricated posting. Close with a lawyer/official-source hand-off that asks BOTH: (1) "[ask your lawyer]" or an official government source whether $X/hour complies with {jurisdiction_name}'s current statutory minimum wage, AND (2) — only when a `special_rates` entry exists on the row but the carve-out honesty gate above left its eligibility unconfirmed from the JD — whether that special-rate carve-out actually applies to this specific role. Omit question (2) when no special rate is in play or its eligibility was already fully confirmed. This table is a reference aid, not legal advice, and does not confirm the current rate — or carve-out eligibility — on its own.]

**Legitimacy corroborator at egregious gaps:** when the advertised figure is far below the table's reference rate (roughly 25%+ below, or below a reference rate that has been in force for years), additionally feed this finding into the Posting Legitimacy assessment as a **corroborator** — an advertised figure that is absurdly low relative to public reference data is a classic ghost-posting/scam marker, independent of whether the reference rate itself is perfectly current. Corroborator only: this signal **never by itself changes** the High Confidence / Proceed with Caution / Suspicious tier; it only adds weight alongside other signals already pointing the same way.

**Phrasing discipline (mandatory):** state only observable facts — the advertised figure, the normalization arithmetic, and the table's non-authoritative reference rate with its citation. Never render this finding as "the employer is breaking the law," an "illegal" posting, a "violation," or as a statement of the *current* legal minimum wage — the table is reference data compiled from cited sources, not a verified, current, authoritative statement of the law, and this mode never gives legal advice. Every rendered instance of this signal must route the candidate to a lawyer or an official source to confirm before relying on it.
