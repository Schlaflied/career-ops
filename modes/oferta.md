...
**12-month staleness gate (mandatory — mirrors the STALENESS RULE in `templates/minimum-wage.yml`):** before selecting a rate, check the row's `as_of` date. This threshold applies uniformly to every row, including rows with no `next_effective` at all — if today's date is more than 12 months past `as_of` and the row carries no verified rate covering today (no `next_rate`, or `next_rate`'s own `next_effective` window has also passed), the row is review-due: treat it as tentative reference data at best, and skip the comparison rather than presenting stale reference data as current. Within a row that is still fresh under this 12-month gate, apply ordinary date-aware selection: use `general_rate` while today's date is before `next_effective`; use `next_rate` once today's date is on or after `next_effective`.
