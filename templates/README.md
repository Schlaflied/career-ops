...
| `statutory-employment-minimums.yml` | `modes/offer-prep.md` (statutory-context notes for sub-statutory terms) | Jurisdiction-keyed table of CATEGORY FLAGS only — which employment-standards clause families (vacation, termination notice, severance, probation limits) a jurisdiction is known to regulate with some statutory floor, and whether it has a whole-provision voiding doctrine for termination clauses (v1 seed: Ontario — flags only, `void_doctrine: true`). Deliberately carries no floor VALUES and no doctrine NARRATIVE (case names, holdings) — those go stale in ways a prompt can't track, so the tool never states them as fact. Prompt-level data reference — no script reads it; local lookup, never online research. Feeds statutory-context flags and targeted lawyer questions that ask what the current figure/doctrine effect is; never a verdict about the candidate's clause. A category absent from a row means the check stays silent for that clause family (the designed "US case"). Contribution rule: no entry without a regulator-grade citable source and an `as_of` verification date. |
