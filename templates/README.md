...
| `minimum-wage.yml` | `modes/oferta.md` (Block G signal 13) | Jurisdiction-keyed table of **non-authoritative reference** minimum-wage figures with date-aware current/next rates (e.g. Ontario $17.60 → pre-announced $17.95 on 2026-10-01) and informational special rates. Prompt-level data reference, agent-judged comparison — no production script reads it (test-all.mjs validates its structure). The rates support currency/hours conversion and comparison arithmetic only; they are never presented as a verified current legal minimum, and every rendered comparison routes the candidate to a `[ask your lawyer]`-style question or an official government source to confirm the actual current statutory rate. Rows older than 12 months (`as_of`) are review-due and treated as tentative or skipped. Contribution rule: no entry without a citable legal source, per-rate effective dates, and an `as_of` verification date. |
